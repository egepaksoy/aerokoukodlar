...
try:
    ...
    threading.Thread(target=frame_send, args=(config, ), daemon=True).start()
    ...
    if broadcast_started.is_set():
        ...
        while not stop_event.is_set():
            ...
            if "iste" in data and gonderildi == False:
                ...
                if distance == None:
                    ...
                    servo_angles = arduino.read_value().split("|")
                    print(servo_angles)

                    client.send_data(data=f"{distance}|{servo_angles[0]}|{servo_angles[1]}")
                    gonderildi = True

            else:
                if "\n" not in data and "|" in data:
                    data = data.strip()
